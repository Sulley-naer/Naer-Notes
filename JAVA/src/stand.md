...
> [!TIP]
> Lanbda 表达式是 Java 8 引入的新特性，可以简化代码，提高代码可读性。
>
> 它允许我们在代码中嵌入一个匿名函数，并在必要时可以传递参数。
>
> 简化了我们传递函数式接口，简化了匿名内部类方法的代码快。

函数式接口，接口有限制：

接口中仅有一个抽象方法,只有符合规范就默认是函数式接口
...
public class Main {
    public static void main(String[] args) {
        ...
        //使用 Lambda 表达式
        Animal animal1 = () -> System.out.println("Animal is eating.");
        animal1.eat();
    }
}
```
...
> [!TIP]
> 方法引用是 Java 8 引入的新特性，可以简化代码，提高代码可读性。
...
> 异常就能很好的解决，它是特殊的 return 能被 try-catch 捕获。

如果没有拦截异常，JVM 会执行默认方式，将异常 原因、信息、位置 打印到控制台，并终止程序的执行。

之类无法抛出比父级还大的异常级别，父级 runtime 自己不能直接 exception 
