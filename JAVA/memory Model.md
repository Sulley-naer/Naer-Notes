...
>
> 栈帧是用来记录 方法中的 局部变量 全局变量是在类中生命的，局部变量保存的方式就是 栈帧。for 循环等等只要有临时变量，栈就会去创建栈帧。在方法结束临时变量无用之后，栈帧自然也会被回收。
...
>
> 堆拥有自己的对象头，不同对象的地址值都不相同，每个堆都是不同的实例/数组。堆通常是 `new` 方法触发的

**对象头**（Object Header）： 存储对象的元数据，包括对象的类信息、哈希码、GC 标记等。
...
> <h2>理解方法区</h2>
>
> java 运行<sup> `new` </sup>加载 `class` 文件后生成的 字节码文件 存放在方法区，方法区也是 JVM 所管理的内存中第二大块，也是垃圾收集器管理的第二个区域。「原来方法区和堆是合并的，现在方法区独立出来」
>
> 在你实例化一个类，它先去开辟一块内存空间，然后在方法区中记录类的信息，包括类的完整类名、修饰符、父类、接口、方法、字段等信息。
>
> 再去 bin 文件中生成了类的字节码。
> 生成完成后 返回地址值 存入栈帧中，栈帧中存放局部变量、方法调用、返回地址等信息。这时候栈才能使用这些方法。
>
> 也就是 方法区是为 `栈帧` 服务的 它将 `栈` 中所需要的 类、变量、方法 独立到其他类，可以让 主进程类 调用 子进程类并返回它所的 `信息` 地址，栈去使用时，它会开另一个 `栈帧` 再去执行。执行结果<sup>`子类栈帧/引用地址`</sup>仍然存储在**方法区栈帧**中。`父类栈帧`中记录的只是 **子类地址**。
>
> <hr>
>
> **类信息**（Class Information）： 包括类的完整类名、修饰符、父类、接口、方法、字段等信息。
>
> **常量**（Constants）： 包括字面值（例如，整型、浮点数、字符串）和符号引用（例如，类名、字段名、方法名）。
>
> **静态变量**（Static Variables）： 属于类而不是实例的变量。
>
> **即时编译器**（JIT）编译后的代码： 即时编译器会将字节码编译为机器码，以提高程序的执行速度。

方法区与 Java 堆不同，它不是用来存储对象的。方法区中的数据通常是全局的，并且在整个程序的生命周期中都存在。
